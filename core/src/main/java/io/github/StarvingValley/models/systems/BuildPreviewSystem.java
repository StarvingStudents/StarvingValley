...
public class BuildPreviewSystem extends IteratingSystem {
    ...
    public BuildPreviewSystem(Camera camera) {
        super(Family.all(BuildPreviewComponent.class, PositionComponent.class).get());
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        WorldLayer worldLayer = Mappers.buildPreview.get(entity).getPreviewLayer();

        GridPoint2 hoveredTile = TileUtils.getHoveredTile(camera);

        ImmutableArray<Entity> blockingEntities = engine.getEntitiesFor(
                Family.all(TileOccupierComponent.class, TileOverlapComponent.class, WorldLayerComponent.class)
                        .get());
